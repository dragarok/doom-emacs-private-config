...
#+end_src
** Dired test unexpected process state 
...
#+end_src
* Clock into non-important tasks 
...
(defun consult-clock-nonimportant-task ()
...
Prioritizes active statuses (READING, STUDYING, PRACTICING) over pending ones."
  ...
  (let* ((tasks (org-ql-select
         ...
         (selected (when sorted
                     (consult--read sorted
                                    :prompt "Clock into mundane/leisure task: "
                                    :require-match t
                                    :sort nil
...
(defvar beancount-helper-file "/Users/alokregmi/Nextcloud/org/personal.beancount"
...
#+end_src
* Add clock time after doing the task 
...
#+end_src
* Open latest downloaded file directly 
