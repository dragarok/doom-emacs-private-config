...
#+end_src
** Trying homebrew git for speedup in emacs
...
(when IS-MAC
  (defun copy-from-osx ()
    "Return macOS clipboard contents as string."
  ...
  (advice-add 'yank :after
              (lambda (&rest _)
                (when (and interprogram-cut-function (current-kill 0 nil))
                  (funcall interprogram-cut-function (current-kill 0 nil))))))
#+end_src
...
#+end_src
** Magit delete branches that are already merged 
...
#+end_src
** Org roam open random note that will have autoread enabled 
...
** Clock out todo and mark it done in one go
...
(bind-key "C-M-s-w" 'toggle-full-screen-window)
...
(after! dap-mode
  (define-key dap-mode-map (kbd "C-s-n") #'dap-next)
  (define-key dap-mode-map (kbd "C-s-k") #'dap-step-out)
  (define-key dap-mode-map (kbd "C-s-o") #'dap-step-in)
  (define-key dap-mode-map (kbd "C-s-c") #'dap-step-continue)
  (define-key dap-mode-map (kbd "C-s-e") #'dap-ui-expressions-add)
  (define-key dap-mode-map (kbd "C-s-r") #'dap-ui-repl)
  (define-key dap-mode-map (kbd "C-s-\"") #'dap-eval-region)
  )
...
(after! org
  ...
  (add-to-list 'org-capture-templates
               '("rl" "Last Week Weekly Review" entry
                 (file+olp+datetree org-weeklyreview-file)
                 (file "~/Nextcloud/org/templates/weeklyreviewtemplate_lastweek.org") :jump-to-captured t :tree-type week))
  )
...
(after! org (add-to-list 'org-capture-templates
                         '("ry" "Daily Review Yesterday" entry (file+olp+datetree org-dailyreview-file)
                           (file "~/Nextcloud/org/templates/dailyreviewtemplate_yesterday.org")
                           :jump-to-captured t)))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(use-package dwim-shell-command
  :ensure t)
#+end_src
* Random Task Selection and Clocking In
...
(defun firefox--get-bookmarks (bookmark-file)
  ...
  (let* ((sql-query
    ...
    root-data))
#+end_src
* Daily and Weekly Review Reminders 

** Test for daily and weekly review reminders
...
;;; Commentary:
;; Single-file implementation of an "evergreen" horizontal notes UI:
;; - opens notes side-by-side
...
;; - folded windows show a vertical title overlay and a thin width
;; - never reuses existing windows (always creates a new split)
;;
;; Key commands:
;;  C-c k r      kairoam-open-note-to-right
...
(defcustom kairoam-folded-width 3
  "Width (columns) for folded windows (thin column)."
  :type 'integer
  :group 'kairoam)
...
(defcustom kairoam-default-expanded-width 80
  "Fallback width to use when calculating expanded window sizes."
  :type 'integer
  :group 'kairoam)
...
(defun kairoam--register-new (win buf pos)
  ...
  (let ((new (make-kairoam-window :window win :buffer buf :position pos :state 'expanded :overlay nil)))
...
(defun kairoam--make-title-overlay (buf title)
  "Create an overlay in BUF that displays TITLE vertically, centered with enhanced styling."
  (with-current-buffer buf
    (save-excursion
      (goto-char (point-min))
      ...
      (let* ((win (get-buffer-window buf))
             (h (if win (window-height win) 20))
             (title-length (length title))
             (padding-lines (max 0 (/ (- h title-length) 2)))
             (top-padding (if (> padding-lines 0) 
                              (make-string padding-lines ?\n) 
                            ""))
             (vertical-title (kairoam--verticalize title))
             (centered-title (concat top-padding vertical-title))
             (ov (make-overlay (point-min) (point-max))))
        ;; Cover entire buffer content with styled vertical title
        (overlay-put ov 'display centered-title)
...
(defun kairoam--resize-window-to (win desired-cols)
  "Resize WIN to DESIRED-COLS (total columns) using multiple strategies for robustness."
  (when (and (windowp win) (window-live-p win))
    (let* ((cur (kairoam--safe-window-width win))
           (delta (- desired-cols cur)))
      (when (/= delta 0)
        ;; Try multiple resize strategies in order of preference
        (condition-case err1
            ;; Strategy 1: Use adjust-window-trailing-edge (more robust for non-selected windows)
            (adjust-window-trailing-edge win delta t)
          (error 
           ;; Strategy 2: Select window first, then resize (mimics Evil's approach)
           (condition-case err2
             ...
             (error 
              ...
              (condition-case err3
                ...
                (error
                 ...
                 (condition-case err4
                     (with-selected-window win
                       (window-resize win delta t t)) ; 4th arg 'ignore' bypasses min size checks
                   (error 
...
(defun kairoam--distribute-sizes ()
  "Compute and set widths for all tracked windows: expanded windows divide frame width equally,
folded windows get `kairoam-folded-width'. Keeps order/positions."
  ...
  (let* ((entries kairoam--registry)
         ...
         (frame-w (frame-width))
         (folded-total (* (length folded) kairoam-folded-width))
         (available (max 1 (- frame-w folded-total)))
         (nexp (max 1 (length expanded)))
         (per-expanded (max 10 (floor (/ available nexp))))) ;; target width for expanded windows
    
    ;; First pass: Force folded windows to their exact width (must be precise)
    (dolist (s folded)
      (let ((win (kairoam-window-window s)))
        (when (and (windowp win) (window-live-p win))
          ;; Force exact width for folded windows, try multiple times if needed
          (let ((attempts 0))
            (while (and (< attempts 3)
                        (/= (window-width win) kairoam-folded-width))
              (kairoam--resize-window-to win kairoam-folded-width)
              (setq attempts (1+ attempts)))))))
    
    ;; Second pass: Resize expanded windows and ensure equal distribution
    ;; We process from left to right to accumulate any rounding errors
    (let ((remaining-width available)
          (remaining-windows (length expanded)))
      (dolist (s expanded)
        (let* ((win (kairoam-window-window s))
               ;; For the last window, use all remaining width to avoid rounding issues
               (target-width (if (= remaining-windows 1)
                                 remaining-width
                               per-expanded)))
          (when (and (windowp win) (window-live-p win))
            (kairoam--resize-window-to win target-width)
            (setq remaining-width (- remaining-width target-width))
            (setq remaining-windows (1- remaining-windows))))))
    
    ;; Third pass: Fine-tune expanded windows to ensure they're actually equal
    ;; Measure actual widths and redistribute if needed
    (when (> (length expanded) 1)
      (let* ((actual-widths (mapcar (lambda (s) 
                                      (window-width (kairoam-window-window s)))
                                    expanded))
             (avg-width (/ (apply #'+ actual-widths) (length actual-widths))))
        ;; If widths vary too much, try to equalize them
        (when (> (- (apply #'max actual-widths) (apply #'min actual-widths)) 2)
          (dolist (s expanded)
            (let ((win (kairoam-window-window s)))
              (when (and (windowp win) (window-live-p win))
                (kairoam--resize-window-to win avg-width)))))))
    
    ;; Final pass: Double-check folded windows stayed at minimum width
    (dolist (s folded)
      (let ((win (kairoam-window-window s)))
        (when (and (windowp win) (window-live-p win)
                   (/= (window-width win) kairoam-folded-width))
    ...
    (kairoam--info "Distributed sizes: expanded=%d folded=%d frame=%d per=%d"
                   (length expanded) (length folded) frame-w per-expanded)))
...
Enforces max-expanded-windows limit by folding farthest windows if needed."
  ...
  (let* ((win (or win (selected-window)))
    ...
    (kairoam--info "Expanded window pos=%s title=%s" (kairoam-window-position s) (kairoam--buffer-title (kairoam-window-buffer s)))
...
(defun kairoam--open-node-at-position (node position)
  ...
  (let* ((file (if (fboundp 'org-roam-node-file) (org-roam-node-file node)
         ...
         (ref-win (if (and (> position 0)
                    ...
                    (selected-window)))
         new-win)
    ;; split to the right of ref-win
    (setq new-win (split-window ref-win nil 'right))
...
(defun kairoam--open-node-with-smart-folding (node pos trigger-pos)
  ...
  (let ((new (kairoam--open-node-at-position node pos)))
    (kairoam--apply-max-expanded-rule trigger-pos)
    (kairoam--distribute-sizes)
    new))
...
(defun kairoam-open-note-to-right ()
  "Interactive: open a node to the right of current selected kairoam window."
  (interactive)
  (kairoam--cleanup-dead)
...
(defun kairoam-open-note-to-left ()
  "Open a node to the left of current window (insert at current pos)."
  (interactive)
  (kairoam--cleanup-dead)
  (let* ((cur-win (selected-window))
         (cur-pos (kairoam--position-of-window cur-win)))
...
(defun kairoam-reset ()
  "Reset all note windows to single expanded view."
  (interactive)
  (delete-other-windows)
  (org-show-all)
  (message "Kairoam layout reset."))
...
#+begin_src elisp
;;; csv.el --- QSV-powered CSV operations for Emacs -*- lexical-binding: t; -*-
...
* Eat semi char mode paste override from evil paste after to eat-yank
...
#+end_src
* Claude code preventing flickers
...
* Font fallback for mac taken from claudemacs (maybe helps claude code )
...
(use-package! claudemacs
  :config
  (define-key prog-mode-map (kbd "s-TAB") #'claudemacs-transient-menu)
  (define-key emacs-lisp-mode-map (kbd "s-TAB") #'claudemacs-transient-menu)
  (define-key text-mode-map (kbd "s-TAB") #'claudemacs-transient-menu)

  ;; setting it in line to ehp catalyst and cce
  ;; (setq claudemacs-program "/Users/alokregmi/workspace/EHP/Catalyst/venv/bin/python")
  ;; (setq claudemacs-program-switches '("/Users/alokregmi/workspace/EHP/Catalyst/cce/capture_3_0/conversations_3_1/conversation_capture_direct.py" "claude"))
  (after! python
    (define-key python-base-mode-map (kbd "s-TAB") #'claudemacs-transient-menu))
  ...
  to load the new symbol and emoji fonts."
    ...
    ;; (set-fontset-font t 'emoji "Noto Emoji" nil 'prepend) ;; If you install Noto Emoji
    ;; Most preferred for emojis -- use your main font here
    (set-fontset-font t 'emoji "Iosevka Nerd Font" nil 'prepend))
