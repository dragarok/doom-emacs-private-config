...
(if IS-MAC
    (setq doom-font (font-spec :family "mononoki" :size 17)
          ...
          doom-serif-font (font-spec :family "Iosevka Nerd Font" :size 17)
...
** Saving text to kill ring emacs even when not pasted anything really

#+begin_src elisp
(setq save-interprogram-paste-before-kill t)
#+end_src

** Remove c-i mapped to tab because moving fwd doesn't work when org mode
For some reason c-i is mapped as tab key so that way org mode will try to use it as org-cycle instead.
...
#+end_src
** Dont create new workspace on new client
...
;; doing dap very efficiently
(bind-key "C-s-n" #'dap-next)
(bind-key "C-s-k" #'dap-step-out)
(bind-key "C-s-o" #'dap-step-in)
(bind-key "C-s-c" #'dap-continue)
(bind-key "C-s-e" #'dap-ui-expressions-add)
(bind-key "C-s-<return>" #'better-jumper-set-jump)
(bind-key "C-s-r" #'dap-ui-repl)
(bind-key "C-s-\"" #'dap-eval-region)
(map! :leader
...
*** Remove quit and enter on org agenda because I want to keep it open permanently
...
#+BEGIN_SRC elisp
(after! org
  (setq org-download-image-dir "~/Nextcloud/org/org-images/"
...
(defun my-log-energy ()
         ...
         (timeout-timer
          (run-at-time "30 sec" nil
                       (lambda ()
                         ...
                         (run-at-time "10 minutes" nil 'my-log-energy))))
         (energy (consult--read energy-levels
                                :prompt "What is your energy level? "
                                :require-match t)))
    ;; Cancel timeout timer if a selection is made
    (when timeout-timer
  ...
  ;; Initial timer at 10 mins, cancel on 30 sec if not selected, 30 mins after if entered
  (run-at-time "10 minutes" nil 'my-log-energy))
#+end_src
* Speedy note taking
...
* Named workspace prioritize before unnamed
...
#+end_src
* Winner undo and redo shortcuts
...
(defun rts--show-task-posframe (heading priority tags)
  "Show selected task in a posframe childframe."
  (require 'posframe)
  (let* ((buffer (get-buffer-create "*Task Selected*"))
         (parent-frame (selected-frame))
         ;; Estimate childframe dimensions in pixels (width: 80 chars, height: 6 lines)
    ...
    ;; Clear the buffer and add task content
    (with-current-buffer buffer
      (erase-buffer)
      (insert (propertize "🎯 TASK SELECTED" 'face 'font-lock-keyword-face))
      (center-line)
      (insert "\n\n")
      (insert (propertize (format "%s" heading) 'face 'font-lock-function-name-face))
      (center-line)
      (insert "\n")
      (insert (propertize (format "Priority: %s" (or priority "No priority")) 'face 'font-lock-variable-name-face))
      (center-line)
      (insert "\n")
      (insert (propertize (format "%s" (if tags (format " :%s:" (string-join tags ":")) "")) 'face 'font-lock-comment-face))
      (center-line)
      (setq buffer-read-only t)
      ;; Set larger font size for the buffer
      (set (make-local-variable 'face-remapping-alist)
           '((default (:height 200) default))))
    ;; Use posframe to display the childframe, centered
...
(defun rts--select-by-probability (tasks)
  ...
  (if (= (length tasks) 1)
      (progn
        (when rts-debug-mode
          (let* ((task (car tasks))
                 ...
                 (points (rts--calculate-task-points task)))
            (rts--debug-log "Single task selected: '%s' (Score: %d)" heading points)))
        (car tasks))
    (let* ((task-points (mapcar (lambda (task)
      ...
      ;; Debug log all candidate tasks and their scores
      (when rts-debug-mode
        (rts--debug-log "=== PROBABILITY SELECTION ===")
        (rts--debug-log "Candidates: %d tasks | Total points: %d | Random point: %d"
                        (length tasks) total-points random-point)
        (dolist (task-point task-points)
          (let* ((task (car task-point))
                 (points (cdr task-point))
                 (heading (org-element-property :raw-value task))
                 (priority (rts--get-task-priority task)))
            (rts--debug-log "  • '%s' [%s] = %d points"
                            heading (or priority "None") points))))
      ...
      (catch 'selected
        (dolist (task-point task-points)
          ...
          (when (>= current-sum random-point)
            (when rts-debug-mode
              (let* ((selected-task (car task-point))
                     (heading (org-element-property :raw-value selected-task))
                     (points (cdr task-point)))
                (rts--debug-log "🎯 WINNER: '%s' (Score: %d)" heading points)))
            (throw 'selected (car task-point))))
        ;; Fallback (shouldn't happen)
        (when rts-debug-mode
          (rts--debug-log "⚠️  WARNING: Fallback selection used"))
        (caar task-points)))))
...
(defun rts--determine-next-status (current-todo)
  "Determine the next status based on current TODO keyword."
  (cond
   ((string= current-todo "TODO") "NEXT")
   ((string= current-todo "TOREAD") "READING")
   ((string= current-todo "TOWATCH") "WATCHING")
   ;; For HABIT and PROJECT, no change
   ((or (string= current-todo "HABIT")
        (string= current-todo "PROJECT")) current-todo)
   ;; Default case - keep current status
   (t current-todo)))

(defun rts--show-task-announcement (message)
  "Show temporary announcement in the middle of screen for 2 seconds."
  (let ((buffer (get-buffer-create "*Task Announcement*")))
    (with-current-buffer buffer
      (erase-buffer)
      (insert message)
      (center-line)
      (setq buffer-read-only t))
    ;; Display the buffer temporarily
...
;; Updated helper functions to properly handle habits and projects
...
Returns nil for habits and projects, t for other tasks."
...
Takes both CURRENT-TODO keyword and TASK element as parameters."
...
(defun rts--display-selected-task (task)
  "Process the selected TASK: change status, clock in, and announce."
  (if task
      (let* ((heading (org-element-property :raw-value task))
             (priority (rts--get-task-priority task))
             (tags (rts--get-task-tags task))
             (marker (org-element-property :org-marker task))
             (current-todo (rts--get-task-todo-keyword task))
             (new-status (rts--determine-next-status current-todo task)))
        ...
        (when marker
          (with-current-buffer (marker-buffer marker)
            (save-excursion
              ...
              ;; Change status only if it should be changed
              (when (and (rts--should-change-status-p task)
                         (not (string= current-todo new-status)))
                (org-todo new-status)
                (when rts-debug-mode
                  (rts--debug-log "Changed status: '%s' -> '%s'" current-todo new-status)))

              ;; Always clock in to the task
              (org-clock-in))))

        ;; Show task in posframe
        (rts--show-task-posframe heading priority tags))
  ...
  (let* ((base-tasks (rts--get-base-tasks))
    ...
    (when filtered-tasks
      (let ((selected-task (nth (random (length filtered-tasks)) filtered-tasks)))
        (when rts-debug-mode
          (let* ((heading (org-element-property :raw-value selected-task))
                 (priority (rts--get-task-priority selected-task)))
            (rts--debug-log "=== RANDOM SELECTION ===")
            (rts--debug-log "Selected from %d filtered tasks: '%s' [%s]"
                            (length filtered-tasks) heading (or priority "None"))))
          ...
          (let ((selected-task (nth (random (length same-time-tasks)) same-time-tasks)))
            (when rts-debug-mode
              (let* ((heading (org-element-property :raw-value selected-task))
                ...
                (rts--debug-log "Selected from %d tasks in %s group: '%s' [%s]"
                                (length same-time-tasks) current-time-group heading (or priority "None"))))
            (rts--display-selected-task selected-task)))
...
#+begin_src elisp
;;; Book Selection Configuration
(defvar rts-book-status-points
  '(("SUMMARIZING" . 5) ("READING" . 3) ("REREADING" . 2) ("TOREAD" . 1))
  "Points assigned to each book status for probability calculation.")

;;; Book Helper Functions

(defun rts--get-book-base-tasks ()
  "Get base book tasks that have 'book' and 'Booxactive' tags."
  (org-ql-select
    (org-agenda-files)
    '(and (todo)
          (tags "book")
...
(defun rts--calculate-book-points (book-task)
  "Calculate points for a book task based on its TODO status."
  (let* ((status (rts--get-task-todo-keyword book-task))
         (points (cdr (assoc status rts-book-status-points))))
    (or points 1))) ; Default to 1 point for unrecognized statuses

(defun rts--select-book-by-probability (books)
  "Select a book from BOOKS based on probability weights."
  (if (= (length books) 1)
      (progn
        (when rts-debug-mode
          (let* ((book (car books))
                 (heading (org-element-property :raw-value book))
                 (status (rts--get-task-todo-keyword book))
                 (points (rts--calculate-book-points book)))
            (rts--debug-log "Single book selected: '%s' [%s] (Score: %d)"
                            heading status points)))
        (car books))
    (let* ((book-points (mapcar (lambda (book)
                                  (cons book (rts--calculate-book-points book)))
                                books))
           (total-points (apply #'+ (mapcar #'cdr book-points)))
           (random-point (random total-points))
           (current-sum 0))

      ;; Debug log all candidate books and their scores
      (when rts-debug-mode
        (rts--debug-log "=== BOOK PROBABILITY SELECTION ===")
        (rts--debug-log "Book candidates: %d books | Total points: %d | Random point: %d"
                        (length books) total-points random-point)
        (dolist (book-point book-points)
          (let* ((book (car book-point))
                 (points (cdr book-point))
                 (heading (org-element-property :raw-value book))
                 (status (rts--get-task-todo-keyword book)))
            (rts--debug-log "  • '%s' [%s] = %d points"
                            heading status points))))
      ...
      (catch 'selected
        (dolist (book-point book-points)
          (setq current-sum (+ current-sum (cdr book-point)))
          (when (>= current-sum random-point)
            (when rts-debug-mode
              (let* ((selected-book (car book-point))
                     (heading (org-element-property :raw-value selected-book))
                     (status (rts--get-task-todo-keyword selected-book))
                     (points (cdr book-point)))
                (rts--debug-log "📚 BOOK WINNER: '%s' [%s] (Score: %d)"
                                heading status points)))
            (throw 'selected (car book-point))))
        ;; Fallback (shouldn't happen)
        (when rts-debug-mode
          (rts--debug-log "⚠️  WARNING: Book fallback selection used"))
        (caar book-points)))))

(defun rts--display-selected-book (book)
  "Process the selected BOOK: clock in and announce."
  (if book
      (let* ((heading (org-element-property :raw-value book))
             (priority (rts--get-task-priority book))
             (tags (rts--get-task-tags book))
             (marker (org-element-property :org-marker book))
             (status (rts--get-task-todo-keyword book)))
        ...
        (when marker
          (with-current-buffer (marker-buffer marker)
            (save-excursion
              (goto-char marker)
              ;; Clock in to the book (no status change for books)
              (org-clock-in))))

        ;; Show book in posframe with book-specific styling
        (rts--show-book-posframe heading priority tags status))

    (message "No books found matching criteria.")))

(defun rts--show-book-posframe (heading priority tags status)
  "Show selected book in a posframe childframe."
  (require 'posframe)
  (let* ((buffer (get-buffer-create "*Book Selected*"))
         (parent-frame (selected-frame))
         ;; Estimate childframe dimensions in pixels (width: 80 chars, height: 6 lines)
         (childframe-pixel-width (* 80 (frame-char-width parent-frame)))
         (childframe-pixel-height (* 6 (frame-char-height parent-frame)))
    ...
    ;; Clear the buffer and add book content
    (with-current-buffer buffer
      (erase-buffer)
      (insert (propertize "📚 BOOK SELECTED" 'face 'font-lock-keyword-face))
      (center-line)
      (insert "\n\n")
      (insert (propertize (format "%s" heading) 'face 'font-lock-function-name-face))
      (center-line)
    ...
    (posframe-show buffer
                   ...
                   :height 10
                   :border-width 2
                   :border-color "blue"  ; Different color for books
                   :accept-focus nil)
...
;;; Main Book Selection Function

(defun random-book-select ()
  "Select a random book from Booxactive books based on status probability."
  (interactive)
  (let ((book-tasks (rts--get-book-base-tasks)))
    (if book-tasks
        (let ((selected-book (rts--select-book-by-probability book-tasks)))
          (when rts-debug-mode
            (let* ((heading (org-element-property :raw-value selected-book))
                   (status (rts--get-task-todo-keyword selected-book)))
              (rts--debug-log "=== RANDOM BOOK SELECTION ===")
              (rts--debug-log "Selected from %d books: '%s' [%s]"
                              (length book-tasks) heading status)))
          (rts--display-selected-book selected-book))
      (message "No active books found with 'book' and 'Booxactive' tags."))))
#+end_src
* Random guitar song to practice selector

#+begin_src elisp
;;; Music Practice Configuration
(defvar rts-music-status-points
  '(("SUMMARIZING" . 5) ("READING" . 3) ("REREADING" . 2) ("TOREAD" . 1)
    ("PRACTICING" . 4) ("LEARNING" . 3) ("REVIEWING" . 2) ("TOLEARN" . 1))
  "Points assigned to each music practice status for probability calculation.")

;;; Music Helper Functions

(defun rts--get-music-base-tasks (instrument-tag)
  "Get base music tasks that have 'musicactive' and INSTRUMENT-TAG.
INSTRUMENT-TAG should be 'guitaractive', 'pianoactive', or nil for any."
  (let ((query (if instrument-tag
                   `(and (todo)
                         (tags "musicactive")
                         (tags ,instrument-tag))
                 '(and (todo)
                       (tags "musicactive")
                       (or (tags "guitaractive")
                           (tags "pianoactive"))))))
    (org-ql-select
      (org-agenda-files)
      query
...
(defun rts--calculate-music-points (music-task)
  "Calculate points for a music task based on its TODO status."
  (let* ((status (rts--get-task-todo-keyword music-task))
         (points (cdr (assoc status rts-music-status-points))))
    (or points 1))) ; Default to 1 point for unrecognized statuses

(defun rts--get-instrument-from-tags (tags)
  "Determine instrument type from task tags."
  (cond
   ((rts--has-tag-p tags "guitaractive") "Guitar")
   ((rts--has-tag-p tags "pianoactive") "Piano")
   (t "Music")))

(defun rts--get-instrument-emoji (instrument)
...
(defun rts--select-music-by-probability (music-tasks instrument-name)
  "Select a music task from MUSIC-TASKS based on probability weights."
  (if (= (length music-tasks) 1)
      (progn
        (when rts-debug-mode
          (let* ((task (car music-tasks))
                 (heading (org-element-property :raw-value task))
                 (status (rts--get-task-todo-keyword task))
                 (points (rts--calculate-music-points task)))
            (rts--debug-log "Single %s task selected: '%s' [%s] (Score: %d)"
                            instrument-name heading status points)))
        (car music-tasks))
    (let* ((task-points (mapcar (lambda (task)
                                  (cons task (rts--calculate-music-points task)))
                                music-tasks))
           (total-points (apply #'+ (mapcar #'cdr task-points)))
           (random-point (random total-points))
           (current-sum 0))
      ...
      ;; Debug log all candidate tasks and their scores
      (when rts-debug-mode
        (rts--debug-log "=== %s PROBABILITY SELECTION ===" (upcase instrument-name))
        (rts--debug-log "%s candidates: %d tasks | Total points: %d | Random point: %d"
                        instrument-name (length music-tasks) total-points random-point)
        (dolist (task-point task-points)
          (let* ((task (car task-point))
                 (points (cdr task-point))
                 (heading (org-element-property :raw-value task))
                 (status (rts--get-task-todo-keyword task))
                 (tags (rts--get-task-tags task))
                 (task-instrument (rts--get-instrument-from-tags tags)))
            (rts--debug-log "  • '%s' [%s] (%s) = %d points"
                            heading status task-instrument points))))
        ...
        (dolist (task-point task-points)
          ...
          (when (>= current-sum random-point)
            (when rts-debug-mode
              (let* ((selected-task (car task-point))
                     (heading (org-element-property :raw-value selected-task))
                     (status (rts--get-task-todo-keyword selected-task))
                     (points (cdr task-point))
                     (tags (rts--get-task-tags selected-task))
                     (task-instrument (rts--get-instrument-from-tags tags))
                     (emoji (rts--get-instrument-emoji task-instrument)))
                (rts--debug-log "%s %s WINNER: '%s' [%s] (Score: %d)"
                                emoji task-instrument heading status points)))
            (throw 'selected (car task-point))))
        ;; Fallback (shouldn't happen)
        (when rts-debug-mode
          (rts--debug-log "⚠️  WARNING: %s fallback selection used" instrument-name))
        (caar task-points)))))

(defun rts--display-selected-music (music-task instrument-name)
  "Process the selected MUSIC-TASK: clock in and announce."
  (if music-task
      (let* ((heading (org-element-property :raw-value music-task))
             (priority (rts--get-task-priority music-task))
             (tags (rts--get-task-tags music-task))
             (marker (org-element-property :org-marker music-task))
             (status (rts--get-task-todo-keyword music-task))
             (actual-instrument (rts--get-instrument-from-tags tags)))

        (when marker
          (with-current-buffer (marker-buffer marker)
            (save-excursion
              (goto-char marker)
              ;; Clock in to the music task (no status change for music)
              (org-clock-in))))

        ;; Show music task in posframe with music-specific styling
        (rts--show-music-posframe heading priority tags status actual-instrument))

    (message "No %s practice tasks found." instrument-name)))

(defun rts--show-music-posframe (heading priority tags status instrument)
  "Show selected music task in a posframe childframe."
  (require 'posframe)
  (let* ((buffer (get-buffer-create "*Music Selected*"))
         (parent-frame (selected-frame))
         (emoji (rts--get-instrument-emoji instrument))
         (border-color (cond
                        ((string= instrument "Guitar") "orange")
                        ((string= instrument "Piano") "purple")
                        (t "green")))
         ;; Estimate childframe dimensions in pixels (width: 80 chars, height: 6 lines)
         (childframe-pixel-width (* 80 (frame-char-width parent-frame)))
         (childframe-pixel-height (* 6 (frame-char-height parent-frame)))
    ...
    ;; Clear the buffer and add music content
    (with-current-buffer buffer
      (erase-buffer)
      (insert (propertize (format "%s %s PRACTICE SELECTED" emoji (upcase instrument))
...
;;; Main Music Selection Functions

(defun random-guitar-select ()
  "Select a random guitar practice task from guitaractive tasks."
  (interactive)
  (let ((guitar-tasks (rts--get-music-base-tasks "guitaractive")))
    (if guitar-tasks
        (let ((selected-task (rts--select-music-by-probability guitar-tasks "Guitar")))
          (when rts-debug-mode
            (let* ((heading (org-element-property :raw-value selected-task))
                   (status (rts--get-task-todo-keyword selected-task)))
              (rts--debug-log "=== RANDOM GUITAR SELECTION ===")
              (rts--debug-log "Selected from %d guitar tasks: '%s' [%s]"
                              (length guitar-tasks) heading status)))
          (rts--display-selected-music selected-task "Guitar"))
      (message "No active guitar practice tasks found with 'musicactive' and 'guitaractive' tags."))))

(defun random-piano-select ()
  "Select a random piano practice task from pianoactive tasks."
  (interactive)
  (let ((piano-tasks (rts--get-music-base-tasks "pianoactive")))
    (if piano-tasks
        (let ((selected-task (rts--select-music-by-probability piano-tasks "Piano")))
          (when rts-debug-mode
            (let* ((heading (org-element-property :raw-value selected-task))
                   (status (rts--get-task-todo-keyword selected-task)))
              (rts--debug-log "=== RANDOM PIANO SELECTION ===")
              (rts--debug-log "Selected from %d piano tasks: '%s' [%s]"
                              (length piano-tasks) heading status)))
          (rts--display-selected-music selected-task "Piano"))
      (message "No active piano practice tasks found with 'musicactive' and 'pianoactive' tags."))))

(defun random-music-select ()
  "Select a random music practice task from any instrument (guitar or piano)."
  (interactive)
  (let ((music-tasks (rts--get-music-base-tasks nil))) ; nil means any instrument
    (if music-tasks
        (let ((selected-task (rts--select-music-by-probability music-tasks "Music")))
          (when rts-debug-mode
            (let* ((heading (org-element-property :raw-value selected-task))
                   (status (rts--get-task-todo-keyword selected-task))
                   (tags (rts--get-task-tags selected-task))
                   (instrument (rts--get-instrument-from-tags tags)))
              (rts--debug-log "=== RANDOM MUSIC SELECTION ===")
              (rts--debug-log "Selected from %d music tasks: '%s' [%s] (%s)"
                              (length music-tasks) heading status instrument)))
          (rts--display-selected-music selected-task "Music"))
      (message "No active music practice tasks found with 'musicactive' tag."))))
#+end_src
...
(use-package obsidian
  ...
  :demand t
  :config
  (obsidian-specify-path "~/Obsidian/MinimalNotes")
  (global-obsidian-mode t)
  :custom
  ;; This directory will be used for `obsidian-capture' if set.
  (obsidian-inbox-directory "02-Fleeting Notes")
  ;; Create missing files in inbox? - when clicking on a wiki link
  ;; t: in inbox, nil: next to the file with the link
  ;; default: t
  ;(obsidian-wiki-link-create-file-in-inbox nil)
  ;; The directory for daily notes (file name is YYYY-MM-DD.md)
  (obsidian-daily-notes-directory "05-Journal")
  ;; Directory of note templates, unset (nil) by default
  (obsidian-templates-directory "99-Meta/Templates")
...
(use-package! md-roam
  ...
  (map! :map doom-leader-map
    "n r o" (λ! (let* ((full-path (buffer-file-name))
                        (pattern "/MinimalNotes/\\(.*?\\)\\.md$")
                        (vault-name "MinimalNotes")
                        obsidian-uri match)
...
(defun toggle-dark-mode ()
  (interactive)
  (if (eq (doom-theme 'doom-monokai-spectrum))
      (consult-theme 'doom-tommorrow-day)
    (consult-theme 'doom-monokai-spectrum)))
#+end_src


** Paste second to last copied info
No need for this. Using ctrl + n can switch to other yanks.
evil-paste-pop and evil-paste-pop-next
#+begin_src emacs-lisp
(defun paste-second-to-last-kill ()
  "Insert the second to last entry in the kill ring."
  (interactive)
  (let ((second-to-last (nth 1 kill-ring)))
    (if second-to-last
        (insert second-to-last)
      (message "No second to last kill available."))))

#+end_src

** Org node for faster org roam
...
(after! org
  (setq org-node-extra-id-dirs '("~/Nextcloud/org"))
...
(use-package consult-web
  ...
  (setq consult-web-sources-modules-to-load '(consult-web-google consult-web-wikipedia consult-web-duckduckgo))
...
(defun switch-dark-mode()
  ...
  (interactive)
  (if (equal doom-theme 'doom-tomorrow-day)
      (progn
        (message "Dark theme enabled")
        (load-theme 'doom-monokai-pro 'noconfirm)
        (doom/reload-theme))
    (progn
...
(global-set-key (kbd "<f5>") 'switch-dark-mode)
#+END_SRC
...
*** Passthrough emacs keys if mac os
...
(defun yabai-execute-commands (&rest commands)
  "Execute yabai commands sequentially until one succeeds."
  (let ((output-buffer (generate-new-buffer "*yabai-output*")))
    (unwind-protect
        (progn
          (while commands
            (let* ((command (car commands))
                   (full-command (concat "yabai -m " command))
                   (process (start-process-shell-command "yabai" output-buffer full-command)))
              ;; (message "Executing yabai command: %s" full-command)
              (set-process-sentinel
               process
               (lambda (proc event)
                 (when (string= event "finished\n")
                   ;; (message "Yabai command output: %s"
          ...
          ;; (message "All yabai commands completed."))
...
(defun yabai-move-on-error (direction move-fn)
  ...
  (condition-case nil
    ...
    (error
     (let ((commands
            (pcase direction
              ("west"  '("window --focus west"
                         "display --focus west"
                         "window --focus stack.prev"
                         "window --grid 1:2:0:0:1:1"))
              ("east"  '("window --focus east"
                         "display --focus east"
                         "window --focus stack.next"
                         "window --grid 1:2:1:0:1:1"))
              ("north" '("window --focus north"
                         "display --focus north"
                         "window --focus stack.next"
                         "window --grid 1:1:0:0:1:1"))
              ("south" '("window --focus south"
                         "display --focus south"
                         "window --focus stack.prev"
                         "window --grid 4:4:1:1:2:2")))))
       ;; (message "Trying to move window with yabai")
       (apply #'yabai-execute-commands commands)))))
...
(use-package! lsp-bridge
  ...
  (after! lsp-bridge
    (push 'lsp-bridge-find-def        +lookup-definition-functions)
    (push 'lsp-bridge-find-type-def   +lookup-type-definition-functions)
    (push 'lsp-bridge-find-impl       +lookup-implementations-functions)
    (push 'lsp-bridge-find-references +lookup-references-functions)
    (push 'lsp-bridge-popup-documentation +lookup-documentation-functions))
  )
#+end_src
...
(defun test-childframe ()
  "Create and display a Vertico-style childframe with larger text using posframe, centered in the Emacs frame."
  (require 'posframe)
  (let* ((buffer (get-buffer-create "*Test Childframe*"))
         (parent-frame (selected-frame))
         ;; Estimate childframe dimensions in pixels (width: 80 chars, height: 10 lines)
         (childframe-pixel-width (* 80 (frame-char-width parent-frame)))
         (childframe-pixel-height (* 10 (frame-char-height parent-frame)))
         ;; Calculate centered position
         (center-x (/ (- (frame-pixel-width parent-frame) childframe-pixel-width) 2))
         (center-y (/ (- (frame-pixel-height parent-frame) childframe-pixel-height) 2))
         (center-position (cons center-x center-y)))
    ;; Clear the buffer and add test content
    (with-current-buffer buffer
      (erase-buffer)
      (insert "This is a test childframe.\nDisplayed at: " (format-time-string "%H:%M:%S"))
      (setq buffer-read-only t)
      ;; Set a larger font size for the buffer
      (set (make-local-variable 'face-remapping-alist)
           '((default (:height 200) default))))
    ;; Use posframe to display the childframe, centered
    (posframe-show buffer
