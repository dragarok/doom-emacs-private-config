...
(after! org (add-to-list 'org-capture-templates
                         '("pn" "New Project" entry
                           ...
                           (file "~/org/templates/newprojtemplate.org"))
                         ))
...
(after! org
  (add-to-list 'org-capture-templates
               '("rw" "Weekly Review" entry
                 ...
                 (file "~/org/templates/weeklyreviewtemplate.org") :jump-to-captured t :tree-type week))

  (add-to-list 'org-capture-templates
               '("rl" "Last Week Weekly Review" entry
                 (file+olp+datetree org-weeklyreview-file)
                 (file "~/org/templates/weeklyreviewtemplate_lastweek.org") :jump-to-captured t :tree-type week))
  )
...
(after! org
  (add-to-list 'org-capture-templates
               '("rm" "Monthly Review" entry
                 ...
                 (file "~/org/templates/monthlyreviewtemplate.org") :jump-to-captured t :tree-type month)))

(after! org
  (add-to-list 'org-capture-templates
               '("rq" "Quarterly Review" entry
                 (file+olp+datetree org-quarterlyreview-file)
                 (file "~/org/templates/quarterlyreviewtemplate.org") :jump-to-captured t :tree-type quarter)))
...
(after! org (add-to-list 'org-capture-templates
                         '("ry" "Daily Review Yesterday" entry (file+olp+datetree org-dailyreview-file)
                           (file "~/org/templates/dailyreviewtemplate_yesterday.org")
                           :jump-to-captured t)))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
#+BEGIN_SRC elisp
(after! org
  (setq org-download-image-dir "~/org/org-images/"
...
(use-package dwim-shell-command
  :ensure t)
#+end_src
* Productivity System (River Flow)

Load the productivity system from lisp/ directory:
...
(defun open-bookmark ()
  (interactive)
  (let ((url (seq-elt (split-string (completing-read "Open: " (browser-bookmarks "~/org/notes/bookmarks.org")) "\n") 2)))
    (browse-url-firefox url)))

(defun open-random-bookmark ()
  "Open a random bookmark from the bookmarks file."
  (interactive)
  (let* ((bookmarks (browser-bookmarks "~/org/notes/bookmarks.org"))
...
The following sections have been moved to lisp/ directory files:
- productivity.el (Random Task Selection, Clock functions, etc.)
- kairoam.el
- booxnoter.el
- beancount_helper.el
- diary-events.el
...
(setq! elfeed-score-file "~/org/elfeed/elfeed-score.el")
#+end_src
...
Problematic since it just full screens the first window.
Using yabai's direct full screen using alt-f and then using winner undo to go back when needed. 
