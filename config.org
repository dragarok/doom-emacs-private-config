...
(defun rts--show-debug-childframe ()
  "Show debug buffer in a childframe."
  (when rts-debug-mode
    (let ((debug-buffer (get-buffer-create "*RTS Debug*")))
      (when (get-buffer-window debug-buffer)
        (delete-window (get-buffer-window debug-buffer)))
...
(defun rts--show-task-announcement (message)
  ...
  (let ((buffer (get-buffer-create "*Task Announcement ChildFrame*")))
    (with-current-buffer buffer
      (erase-buffer)
      (insert message)
      (center-line)
    ...
    ;; Show debug info in childframe if enabled
    (rts--show-debug-childframe)))
...
(defun rts--display-selected-task (task)
  ...
  (if task
      (let* ((heading (org-element-property :raw-value task))
        ...
        ;; Show announcement
        (let ((announcement (format "🎯 SELECTED: %s [%s]%s"
                                    heading
                                    (or priority "No priority")
                                    (if tags (format " :%s:" (string-join tags ":")) ""))))
          (rts--show-task-announcement announcement)))
