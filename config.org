...
(bind-key "C-M-s-b" 'consult-yank-from-kill-ring)
(bind-key "C-M-s-s" 'my/org-roam-dailies-goto-start-of-week)
(bind-key "C-M-s-\"" 'evil-avy-goto-char-timer)
(bind-key "C-M-s-w" 'process-inbox-item)
...
(after! org
  (setq my-org-categories '("Inbox" "Events" "ToTheMoon" "ToImprove" "Hobby" "Fitness" "Normal" "Mundane" "Celebration" "EHP"))
  (defun my-select-org-category (&rest args)
    (completing-read "CATEGORY value: " my-org-categories))
  (add-to-list 'org-property-set-functions-alist
               '("CATEGORY" . my-select-org-category))
  )
#+END_SRC

** Set default for categories and effort estimates
...
(after! org
  ...
  (defun my-select-org-category (&rest args)
    (completing-read "CATEGORY value: " my-org-categories))
  (defun my-select-org-effort (&rest args)
    (completing-read "Effort value: " '("0:10" "0:15" "0:30" "0:45" "1:00" "1:30" "2:00" "3:00" "5:00" "10:00")))
  (add-to-list 'org-property-set-functions-alist
               '("CATEGORY" . my-select-org-category))
  (add-to-list 'org-property-set-functions-alist
               '("Effort" . my-select-org-effort))
  )
...
*** Org roam dailies goto start of week

#+begin_src emacs-lisp
(after! org
  (defun my/org-roam-dailies-goto-start-of-week ()
    "Go to the Org-roam daily for the first day (Monday) of the current week."
    (interactive)
    (let* ((today (current-time))
           (weekday (format-time-string "%u" today)) ; %u is day of week [1,7] where Monday is 1
           (days-to-subtract (% (- (string-to-number weekday) 1) 7))
           (start-of-week (time-subtract today (days-to-time days-to-subtract))))
      (org-roam-dailies--capture start-of-week t)))
  )
